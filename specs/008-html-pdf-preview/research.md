...
- **Decision**: `dist/deck.pdf` および `dist/pdf-pages/` は追跡しない。既存 `.gitignore` の `dist/*`（`!dist/sample-deck.pptx` のみ例外）で自動的に無視されるため、追加設定は不要（FR-011）。
- **Rationale**: 既存規約をそのまま満たす。新規 `.gitignore` 行を足さないのが最小変更。
- **Alternatives considered**: PDF を追跡（PPTX のように）→ バイナリ肥大・レビューノイズ。確認用途では不要。
