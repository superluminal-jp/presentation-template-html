...
**Input**: spec.md(US1〜US3)/ plan.md / research.md / data-model.md(18種台帳)/ contracts/appendix-contract.md(INV-1〜12)
...
**Goal**: 台帳 18 種すべてをデッキ付録で 1 回以上実演する(不足は 区切り + DADS 公式 6 種)。

**Independent Test**: `index.html`/生成 `slides/` だけを閲覧し、18 種が判別可能に出現(SC-001 / INV-1)。
...
- [ ] T004 [P] [US1] Reconcile component-count text in `index.html`: タイトル meta「12コンポーネント」→「18コンポーネント」、ダッシュボードタイル「コンポーネント 12 / 再利用可」→「18 / 自作12+公式6」(FR-010 / SC-005)
...
**Checkpoint**: 18 種が付録で実演され、構造・a11y 検証が緑。US1 は単独で MVP として成立。
...
- [ ] T016 [US3] Add appendix slide(s) to `scripts/pptx/build-pptx.mjs`: コールアウト/バッジ/タグ/ボタン/データテーブル(`addTable`)/プログレス/統計/凡例/区切り/チェックリスト/引用/kv と DADS 公式 6 種を、ネイティブ図形+テキスト+ネイティブ表で近似(SVG 多系列チャートは簡略/省略可)。`builders` 配列へ追加し chrome/採番に載せる
...
- [ ] T019 Run the full acceptance bundle from `contracts/appendix-contract.md`(lint:tokens / check:crossrefs / check:coverage / check:slides / test:a11y / test:visual / test:print / build:pptx + 参照0 grep)and confirm SC-001〜005 satisfied
...
1. **MVP = US1**: 付録に 18 種を揃えるだけで「単一情報源」の中核価値が成立(独立ページはまだ残っていてもよい)。
