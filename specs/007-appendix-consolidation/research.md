...
**Decision**: 既存付録 A〜E を活かし、追加 = **区切り(divider)**・**DADS 公式 6 種**・および分析で採用した**新規自作 2 種(バナー〔通知・緊急〕/ステップナビゲーション)**。計 20 種(自作 14 + 公式 6)。
...
- 付録E(グラフ事例): SVG チャート群 ✓
- **未実演**: 区切り(divider)= 既存自作 12 種のうち唯一の欠落 / DADS 公式 6 種(カード・箇条書きリスト・チェックボックス・ラジオ・プログレスインジケーター・パンくず)
- **新規追加(分析採用)**: バナー(通知・緊急)/ステップナビゲーション(自作・トークン準拠。DS の同名パターンに着想) → 自作は 12+2=14 種

**Rationale**: 重複実演を避ける(FR-002)。既存の実演を壊さず追加のみで 20 種網羅(SC-001)。
...
**Decision**: デッキ本文の「12コンポーネント」表記を **20** に更新する。該当箇所は少なくとも 2 つ:
- タイトルスライド meta の「12コンポーネント」
- ダッシュボードタイル「コンポーネント 12(再利用可)」

**Rationale**: FR-010/SC-005(実体との整合)。自作 14(既存12+バナー・ステップナビ)+ 公式 6 = 20。
