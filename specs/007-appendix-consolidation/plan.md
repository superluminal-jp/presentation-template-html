...
独立ページ `components.html` を廃止し、その全コンポーネント実例をデッキ本体(`index.html`)の付録スライドに一本化する。現状、自作 12 種は付録 A〜C に概ね実演済みだが **区切り(divider)** が未実演、**DADS 公式 6 種は付録に未実演**(`components.html` のみ)。さらに分析(外部3ソース検証)を受け、スライド作成に有効な**自作の新規 2 種(バナー〔通知・緊急〕/ステップナビゲーション)を追加**し、対象を計 20 種(自作 14 + 公式 6)とする。よって「付録の拡張(不足分+新規の追加)→ 独立ページと参照の除去 → 数値表記の整合 → PPTX への付録追加」の順で進める。正典は `index.html`、`slides/*.html` は再生成物。
...
**Scale/Scope**: デッキ約 21→拡張後スライド、コンポーネント 20 種(自作 14 + DADS 公式 6)、PPTX 16→拡張後スライド。
