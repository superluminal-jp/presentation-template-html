...
### Won't(フェーズ1では対象外)
- ~~PowerPoint への実変換(フェーズ2)~~ → **実装済み**(feature 011: DOM 実測抽出 + pptxgenjs。`npm run build:pptx`)。アニメーション、CMS 連携、多言語切替は引き続き対象外
...
|---|---|---|---|---|
| 素 HTML/CSS(採用=正本) | ◎ `tokens.css` を直接参照 | ◎ | ◎(DOM 実測抽出で同一正本から生成 — feature 011) | 採用 |
| Marp(Markdown→スライド) | ○ テーマ CSS に取込可 | ◎ 著者体験良 | ×(PPTX は画像化=非編集) | 補助候補 |
| reveal.js | ○ | ○ | × | 不採用(FW 依存) |
| python-pptx 単独 | △ 値の手写経になりがち | △ | ◎ | 不採用(「手写経になりがち」が実際に起きた — 下記) |
| Slidev | ○ | ○(開発者向け) | × | 不採用 |

**結論**: 視覚の正本は素 HTML/CSS(DS トークンを CSS 変数でそのまま参照でき、更新追従とホームページ配色踏襲を最も低コストに満たす)。Marp は Markdown での量産が必要になった場合の著者レイヤーとして、同じテーマ CSS を共有する形で追加検討。

**フェーズ2 の確定(feature 011)**: 編集可能な PowerPoint は、当初案の「python-pptx で
レイアウト対応表からプログラム生成」ではなく、**正本 HTML の DOM 実測抽出**で生成する
(`npm run build:pptx`、Node + Playwright + pptxgenjs)。

当初案を採らなかった理由は、この表が「python-pptx 単独」に付けた△「値の手写経になりがち」が、
対応表方式でも実際に起きたため。手書きの変換器は正本と独立した第二の正本となり、正本 28 枚に
対し 20 枚しか変換されず(8 枚が欠落)、日本語リテラル 208 個が二重管理となり、表紙メタが
陳腐化した。DOM 実測抽出は、算出済み矩形と計算済みスタイルだけを読むことで対応表そのものを
不要にし、正本を唯一の情報源に戻す。乖離は `npm run check:pptx` が `verify` の中で検出する。
...
- ~~PPT 変換手段はフェーズ2で確定(python-pptx を第一候補)(M)。~~ → **確定**: DOM 実測抽出 + pptxgenjs(feature 011)。新規ランタイム依存なし。
